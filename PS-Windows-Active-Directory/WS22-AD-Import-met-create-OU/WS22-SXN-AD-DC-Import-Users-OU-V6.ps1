...
function Get-VerzamelOUName
{
    param(
        [string]$Value
    )

    if ($Value -match 'VerzamelOU=([^,]+)')
    {
        return $Matches[1]
    }
...
function Get-GebruikersOUName
{
    param(
        [string]$Value
    )

    if ($Value -match 'GebruikersOU=([^,]+)')
    {
        return $Matches[1]
    }
...
foreach ($OU in $VerzamelOUs)
{
    try
    {
        $VerzamelOUName = Get-VerzamelOUName $OU

        if (:IsNullOrWhiteSpace($VerzamelOUName))
        {
            continue
        }
...
foreach ($OUInfo in $OUCombinaties)
{
    try
    {
        $VerzamelOUName = Get-VerzamelOUName $OUInfo.VerzamelOU
        $GebruikersOUName = Get-GebruikersOUName $OUInfo.GebruikersOU
        ...
        if (:IsNullOrWhiteSpace($GebruikersOUName))
        {
            continue
        }
        ...
        {
            Write-Host "[BESTAAT AL] $VerzamelOUName\$GebruikersOUName" -ForegroundColor Yellow
        }
    }
    catch
    {
        Write-Warning "Fout bij aanmaken van OU $GebruikersOUName"
        Write-Warning $_.Exception.Message
    }
}
...
foreach ($User in $Users)
{
    try
    {
        $VerzamelOUName = Get-VerzamelOUName $User.VerzamelOU
        $GebruikersOUName = Get-GebruikersOUName $User.GebruikersOU

        $UserOU = "OU=$GebruikersOUName,OU=$VerzamelOUName,$DomainDN"

        $OUBestaat = Get-ADOrganizationalUnit `
            -LDAPFilter "(ou=$GebruikersOUName)" `
            -SearchBase "OU=$VerzamelOUName,$DomainDN" `
            -ErrorAction SilentlyContinue

        if (-not $OUBestaat)
        {
            Write-Warning "Doel OU bestaat niet: $UserOU"
            continue
        }
        ...
        Write-Host "[AANGEMAAKT] $SamAccountName" -ForegroundColor Green
    }
    catch
    {
