...
#
$VMWARE_Tools_Installer = "$env:USERPROFILE\Downloads\VMware\VMware-tools-x64-Latest.exe"
#
Write-Host 'Bepalen Latest'
#
uri = 'https://packages.vmware.com/tools/releases/latest/windows/x64/'
# Haal de directory listing op
$response = Invoke-WebRequest -Uri $uri
# Zoek de juiste .exe
$exe = $response.Links |
    Where-Object {
        $_.href -match '^VMware-tools.*\.exe$'
    } |
    Select-Object -First 1
if (-not $exe) {
    throw 'Geen VMware Tools .exe gevonden'
}
# Bouw de volledige download-URL
$downloadUrl = [uri]::new($uri, $exe.href).AbsoluteUri
#
Write-Host 'Downloading'
#
Invoke-WebRequest -Uri $downloadUrl -OutFile $VMWARE_Tools_Installer
#
