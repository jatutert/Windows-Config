...
Write-Host "Version 25" -ForegroundColor Green
...
$osInfo = Get-CimInstance -ClassName Win32_OperatingSystem
if ($osInfo.ProductType -eq 1) {
    reg add HKCU\Software\Policies\Microsoft\Windows\Explorer /v HideFeeds /t REG_DWORD /d 1 /f
    reg add HKLM\Software\Policies\Microsoft\Windows\Explorer /v HideFeeds /t REG_DWORD /d 1 /f
}
#
#
#   ###########################################
...
Write-Host "[Powershell] Active Directory modules activeren..." -ForegroundColor White
